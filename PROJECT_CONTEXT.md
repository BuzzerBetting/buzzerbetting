...
12. Third instance of the §9 pattern (2026-08-19) — the DO server was fully disconnected from git for ~2 weeks, not just "possibly drifted"

Shipped three fixes this session (commit `e66ee32`): a clickable "C054 + N" multi-account bet display on the Bet Tracker/Dashboard (new `GET /api/ledger/bets/:id/legs` endpoint — confirmed first that odds are stored once per bet, not per leg, so the modal correctly shows one shared odds value rather than implying per-account odds exist), the Calculator role's blanket `/api/ledger/*` block now exempts `/fotmob-leagues` specifically (was breaking Edit Leagues on Today's Matches) with the add/remove endpoints also opened to every role, and a "Placed" date column on the Dashboard's "Open bets to be settled" widget. All three verified end-to-end locally (real backend + real `ledger.js` proxy + the actual `index.html` executed in a real DOM against live HTTP calls) before pushing.

Netlify auto-deployed the new `index.html` immediately, so the clickable "+N" UI went live right away — but clicking it hit `GET /bets/:id/legs` on the DO server, which didn't exist yet, and failed with what looked like a generic "Network error" (Express's default HTML 404 body fails frontend `res.json()` parsing, landing in the same catch block as an actual network failure — worth remembering as a symptom, this failure mode doesn't announce itself as a 404).

The owner tracked it down directly (SSH, not through Claude Code — sandboxed Bash blocked an SSH attempt as an outward-facing production action) and found something more specific than "the server can drift": **this particular DO server had been fully disconnected from `git pull` since around August 3rd** — roughly two weeks of GitHub pushes never reaching production. Root cause of the disconnection itself: `git pull` had been silently failing/no-op-ing because of local changes blocking the merge — a stale, pre-`/api/ledger`-mount `package.json`/`server.js` left over from before the §2 fix even landed, plus `ledger-db.js` and `ledger-routes.js` sitting on the server as untracked files from manual edits, never actually committed there. Fixed by the owner: backed everything up, diffed to confirm GitHub's versions were strictly newer with nothing locally unique worth keeping, discarded the tracked changes, moved the untracked files to a backup folder, pulled clean (landed exactly on `e66ee32`), restarted PM2, confirmed working.

Practical upshot for future sessions: §9/§11's "always check the live file directly, don't assume the repo is current" is not a hypothetical caution for this project — it has now recurred a third time, and this instance shows the drift can be long and silent (weeks, not hours) with a specific, avoidable cause (uncommitted/untracked local edits quietly blocking every future pull). Before trusting that a just-pushed fix is actually live, check `git log -1` (or equivalent) on the DO server itself, not just GitHub — and if a "should be live" fix isn't behaving, a generic-looking error (e.g. "Network error" from a JSON-parse failure on an HTML 404 body) is worth treating as a possible symptom of exactly this, not just a bug in the new code.