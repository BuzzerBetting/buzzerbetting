...
//   DISCORD_BOT_TOKEN            - the bot token (Reset Token in the Discord dev portal)
...
//
// The bot only acts when the site has the toggle on AND an account selected (Corners
// tracker section) — otherwise the server replies status:"disabled" and nothing is booked.
...
const EMOJI = { booked: '✅', skipped: '⚠️', disabled: '💤', error: '❌', duplicate: '🔁' };

const client = new Client({
  intents: [GatewayIntentBits.Guilds, GatewayIntentBits.GuildMessages, GatewayIntentBits.MessageContent],
});

client.once('ready', () => console.log(`[discord-corners] logged in as ${client.user.tag}; watching channel ${CHANNEL_ID}`));
...
client.on('messageCreate', async (msg) => {
  ...
  for (const img of images) {
    try {
      ...
      const status = d.status || (d.ok ? 'error' : 'error');
