// netlify/functions/oc-dragonbet-football-boost-ev.js
//
// Thin proxy to the DO server's /api/dragonbet-football-boost-ev (server.js) — DragonBet's
// individual-match football boosts other than a plain win-acca/horse-double, written by
// oc-scraper's dragonbet_football_boost_scan.py via the shared oc/football_boost_markets.py
...
exports.handler = async (event) => {
  ...
  try {
    const res = await fetch(`http://${DO_HOST}:${DO_PORT}/api/dragonbet-football-boost-ev`, { method: 'GET' });
