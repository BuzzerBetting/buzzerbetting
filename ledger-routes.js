...
// Temporary "assist -> header" corner-bet prediction log — see corner-model/predictionLog.js.
// Same optional-require pattern as cornerModel above; its absence just means no logging, not a
// broken match-predictions route.
const predictionLog = (() => {
...
router.get('/ones-to-watch', (req, res) => {
  try {
    const rows = db.prepare(
      `SELECT id, match_id AS matchId, match, kickoff, kickoff_date AS kickoffDate, market, selection, fair, base_fair AS baseFair, conf, bookie, odds, ev, source, bfex_lpm AS bfexLpm, bfex_matched AS bfexMatched, state
         FROM ones_to_watch
        WHERE state != 'removed'
...
router.get('/ones-to-watch/removed', (req, res) => {
  try {
    const rows = db.prepare(
      `SELECT id, match_id AS matchId, match, kickoff, kickoff_date AS kickoffDate, market, selection, fair, base_fair AS baseFair, conf, bookie, odds, ev, source, bfex_lpm AS bfexLpm, bfex_matched AS bfexMatched, state, updated_at AS removedAt
         FROM ones_to_watch
        WHERE state = 'removed'
...
router.post('/ones-to-watch/ingest', (req, res) => {
  try {
    ...
    const tx = db.transaction(() => {
      ...
      const ins = db.prepare(
        `INSERT INTO ones_to_watch (match_id, match, kickoff, kickoff_date, market, selection, fair, base_fair, conf, bookie, odds, ev, source, bfex_lpm, bfex_matched, state, created_at, updated_at)
         VALUES (@match_id, @match, @kickoff, @kickoff_date, @market, @selection, @fair, @base_fair, @conf, @bookie, @odds, @ev, @source, @bfex_lpm, @bfex_matched, 'pending', @now, @now)
      ...
      for (const r of incoming) {
        ...
        ins.run({
          ...
          kickoff_date: b.kickoffDate || r.kickoffDate || null,
